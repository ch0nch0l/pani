...
class AlarmViewHolder(view: View, private val delegate: Delegate) : BaseViewHolder(view) {
  ...
  override fun bindData(data: Any) {
    ...
    itemView.item_alarmrecord_btn_delete.setOnClickListener { view ->
      ...
      alertDlg.setPositiveButton("হ্যাঁ") { dialog: DialogInterface, which: Int -> delegate.onConfirm(alarmModel) }
      alertDlg.setNegativeButton("না") { dialog, which -> dialog.dismiss() }
      alertDlg.setMessage(String.format("আপনি কি এই অ্যালার্মটি মুছে ফেলতে চান?"))
      alertDlg.show()
    }
  }

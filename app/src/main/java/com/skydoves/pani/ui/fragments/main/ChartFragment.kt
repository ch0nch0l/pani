...
import com.skydoves.pani.ui.customViews.MyMarkerView
...
class ChartFragment : Fragment(), OnChartValueSelectedListener {
  ...
  internal fun DateMoveButton(v: View) {
    ...
    when (v.id) {
      ...
      R.id.chart_ibtn_next -> if (dateCount == -DayNum)
        Toast.makeText(context, "আগামী সপ্তাহের রেকর্ড পাওয়া যাচ্ছে না", Toast.LENGTH_SHORT).show()
      else {
        dateCount += 7
        if (dateCount == -DayNum)
  ...
  private fun initializeChart(dayCount: Int) {
    ...
    val dataset = LineDataSet(entries, "পানির পরিমাণ")
